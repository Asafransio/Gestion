...
public class BajaParaEn extends Frame implements WindowListener, ActionListener
{
	private static final long serialVersionUID = 1L;
	Label lblEdificioFactura = new Label("Parada a borrar:");
	Choice choLineas = new Choice();
	...
	BajaParaEn()
	{
		setTitle("Baja de la relacion");
		setLayout(new FlowLayout());
		// Montar el Choice
		choLineas.add("Seleccionar uno...");
		...
		try {
			...
			while (rs.next()) 
			{
				choLineas.add(rs.getInt("idLineaFK")+
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if(objetoPulsado.equals(btnSi))
		{
			...
			Connection con = conectar(); 
			// Borrar
			String[] Linea=choLineas.getSelectedItem().split("-");
			int respuesta = borrar(con, Integer.parseInt(Linea[0]));
			// Mostramos resultado
			if(respuesta == 0)
			{
				System.out.println("Borrado de relacion correcto");
			}
			else
			{
				System.out.println("Error en Borrado de relacion");
			}
			// Actualizar el Choice
			choLineas.removeAll();
			choLineas.add("Seleccione la parada");
			String sqlSelect = "SELECT * FROM paraen";
			try {
				// CREAR UN STATEMENT PARA UNA CONSULTA SELECT
				Statement stmt = con.createStatement();
				...
				while (rs.next()) 
				{
					choLineas.add(rs.getString("idLineaFK")+
							"-"+rs.getString("idParadaFK"));
				}
				rs.close();
				stmt.close();
	...
	public int borrar(Connection con, int idLineaFK)
	{
		int respuesta = 0;
		String sql = "DELETE FROM paraen WHERE idLineaFK = " + idLineaFK;
